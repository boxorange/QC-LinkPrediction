...
output_path = os.path.expanduser("~/QC-LinkPrediction/data/SEMNET/embeds/tsv/mean_pooled_keyword_embedding.tsv)"
with open(output_path, 'w+') as fout:
    for i in mean_pooled_keyword_embedding:
        fout.write(i + '\n')
 
output_path = os.path.expanduser("~/QC-LinkPrediction/data/SEMNET/embeds/tsv/max_pooled_keyword_embedding.tsv)"
with open(output_path, 'w+') as fout:
    for i in max_pooled_keyword_embedding:
        fout.write(i + '\n')

output_path = os.path.expanduser("~/QC-LinkPrediction/data/SEMNET/pt/mean_pooled_embedding)"
feature_dict = {'entity_embedding': torch.tensor(mean_pooled_embedding, dtype=torch.float32)}
torch.save(feature_dict, output_path)

output_path = os.path.expanduser("~/QC-LinkPrediction/data/SEMNET/pt/max_pooled_embedding)"
