...
from baseline_models.BUDDY.hashing import ElphHashes
from torch_geometric.utils import to_undirected
from torch_sparse import coalesce
import scipy.sparse as ssp
from baseline_models.BUDDY.heuristic import RA

import torch_sparse
from torch_geometric.nn.conv.gcn_conv import gcn_norm
import numpy as np
from baseline_models.BUDDY.utils import get_same_source_negs
...
class HashDataset(Dataset):
    ...
    def __init__(
        ...
        if args.model == 'ELPH':  # features propagated in the model instead of preprocessed
            ...
            # [GP] - add a node embedding path. 07/12/2024
            self.x = self._preprocess_node_features(args.embed_path, data, self.edge_index, self.edge_weight, args.sign_k)
    ...
    # [GP] - add a node embedding path. 07/12/2024
    def _preprocess_node_features(self, emb_path, data, edge_index, edge_weight, sign_k=0):
        """
        ...
        if self.load_features and os.path.exists(feature_name):
            ...
            # [GP] - add a node embedding path. 07/12/2024
            emb_path = os.path.expanduser(emb_path)
            x = torch.load(emb_path)
            x = x['entity_embedding'].to(edge_index.device)
...
def make_train_eval_data(args, train_dataset, num_nodes, n_pos_samples=5000, negs_per_pos=1000):
    ...
    subgraph_cache_name = f'{ROOT_DIR}/dataset/{dataset_name_save}/train_eval_negative_samples_{negs_per_pos}_subgraph_featurecache.pt'
    ...
    if os.path.exists(subgraph_cache_name):
        ...
        if args.max_hash_hops == 3:
            hash_name = f'{ROOT_DIR}/dataset/{dataset_name_save}/{file_stub}_elph__train_3hop_hashcache.pt'
        else:
            hash_name = f'{ROOT_DIR}/dataset/{dataset_name_save}/{file_stub}_elph__train_hashcache.pt'
