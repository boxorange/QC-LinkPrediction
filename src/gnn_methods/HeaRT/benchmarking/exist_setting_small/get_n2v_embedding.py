...
# [GP] - added data dir. 06/14/2024
def read_data(data_name, neg_mode, data_dir):
    data_name = data_name
    ...
    # [GP] - feature embeddings are not used for node2vec embedding generation. 06/21/2024
...
def main():
    ...
    # parser.add_argument('--embed_name', type=str, default='')
    ...
    # data = read_data(args.data_name, args.neg_mode, data_dir, embed_name)
    data = read_data(args.data_name, args.neg_mode, data_dir)
    

    train_edge = torch.transpose(data['train_pos'], 1, 0)
    edge_index = torch.cat((train_edge, train_edge[[1,0]]), dim=-1)

    # [GP] - set the output path. 06/14/2024
    # edge_index = dir_path+'/dataset' 
    # save_path = dir_path + '/dataset/'+args.data_name+'-n2v-embedding.pt'
    save_path = os.path.join(output_dir, args.data_name + '-n2v-embedding.pt')
