...
class Logger(object):
    ...
    def print_statistics(self, run=None, log_data=[]):
        if run is not None:
            ...
            log_data.append(f'Highest Train: {result[:, 0].max():.2f}')
            log_data.append(f'Highest Valid: {result[:, 1].max():.2f}')
            log_data.append(f'  Final Train: {result[argmax, 0]:.2f}')
            log_data.append(f'   Final Test: {result[argmax, 2]:.2f}')
            ...
            # [GP] - save results log to file.	
            log_data.append(f'All runs:')
            
            r = best_result[:, 0].float()
            print(f'Highest Train: {r.mean():.2f} ± {r.std():.2f}')
            
            # [GP] - save results log to file.	
            log_data.append(f'Highest Train: {r.mean():.2f} ± {r.std():.2f}')
            ...
            # [GP] - save results log to file.	
            log_data.append(f'Highest Valid: {r.mean():.2f} ± {r.std():.2f}')
            ...
            # [GP] - save results log to file.	
            log_data.append(f'  Final Train: {r.mean():.2f} ± {r.std():.2f}')
            ...
            # [GP] - save results log to file.	
            log_data.append(f'   Final Test: {r.mean():.2f} ± {r.std():.2f}')
