...
def get_threshold(val_pred, val_true):
    
    # Compute ROC curve
    fpr, tpr, thresholds = roc_curve(val_true, val_pred)

    # Method 1: Find the optimal threshold by maximizing Youden's J statistic (sensitivity + specificity - 1)
    J = tpr - fpr
    ...
    '''
    # Method 2: Find the optimal threshold by minimizing the distance to the top-left corner of the ROC curve
    optimal_idx = np.argmin(np.sqrt(fpr**2 + (1-tpr)**2))
    optimal_threshold = thresholds[optimal_idx]

    print(f"Optimal Threshold (Top-Left Corner): {optimal_threshold}")
    '''
